...
<h1>AntherListen Online</h1>
<p>A music client that fuses Spotify and YouTube Music into one seamless experience</p>

[![Latest release](https://img.shields.io/github/v/release/anagh9090/AntherListen-Online?style=for-the-badge)](https://github.com/anagh9090/AntherListen-Online/releases/latest)
[![GitHub license](https://img.shields.io/github/license/anagh9090/AntherListen-Online?style=for-the-badge)](https://github.com/anagh9090/AntherListen-Online/blob/main/LICENSE)

</div>

## What is AntherListen?

**AntherListen** is an Android music client that brings together the best of Spotify and YouTube Music. It uses your Spotify account to power personalized recommendations, search, and home content — while streaming audio through YouTube Music.
...
- **No Spotify Premium required** — AntherListen uses Spotify's data APIs (not streaming), so a free Spotify account is all you need
...
<div align="center">
<a href="https://github.com/anagh9090/AntherListen-Online/releases/latest/download/AntherListen.apk"><img src="https://github.com/machiav3lli/oandbackupx/blob/034b226cea5c1b30eb4f6a6f313e4dadcbb0ece4/badge_github.png" alt="Get it on GitHub" height="82"></a>
</div>

> **First time here?** Tap the badge above or go to the [Releases page](https://github.com/anagh9090/AntherListen-Online/releases), then download the **AntherListen.apk** file and open it on your Android device. You may need to allow installation from unknown sources in your phone's settings.


## How the Spotify Integration Works

AntherListen connects to your Spotify account through a built-in WebView login — no developer setup or Client ID required. Here's what happens under the hood:

1. **Authentication** — You log in with your regular Spotify credentials (email, Google, Facebook, or Apple) directly inside the app. AntherListen extracts session cookies and generates access tokens using TOTP, keeping you logged in without manual token management.
2. **Data layer** — AntherListen communicates with Spotify primarily through GraphQL endpoints (for playlists, liked songs, artist details, albums, new releases, and search) with REST API fallbacks for top tracks and top artists. GraphQL avoids the aggressive rate limits that affect REST endpoints.
3. **Home screen** — When "Use Spotify for Home" is enabled, AntherListen builds a personalized home feed from your top tracks, top artists, playlists, and new releases. Enable "Spotify only" to hide all YouTube-based sections for a fully Spotify-driven experience.
...
6. **Queue generation** — When you play a Spotify-sourced song, AntherListen's recommendation engine builds a queue by:
...
When the Qobuz toggle is enabled (Settings → Integrations → Spotify → "Use Qobuz for lossless playback"), AntherListen routes audio through Qobuz's FLAC catalog instead of YouTube Music's lossy AAC streams. The integration is fully opt-in and falls back to YouTube Music whenever Qobuz can't deliver — there's no playback interruption either way.

1. **Match resolution** — For every track about to play, AntherListen looks up the song on Qobuz. Spotify-sourced tracks include the **ISRC** (the universal track identifier — the same ISRC points to the same recording across Spotify, Qobuz, Tidal, etc.) which produces an exact, deterministic match. YT-native tracks fall back to fuzzy title/artist/album matching using the cached song metadata.
2. **Backend cycling** — Qobuz is accessed through three independent open community resolvers (Monokenny, Jumo, Squid). The primary backend is configurable; if it returns a preview, captcha challenge, or any other failure, AntherListen automatically retries on the alternates before giving up. Backends that hit a captcha are skipped for five minutes to avoid wasted retries.
...
4. **Quality tier downgrade** — When the saved match knows the track only exists at CD quality on Qobuz (not Hi-Res), AntherListen caps the requested quality automatically to avoid the wasted "preview returned" round-trip.
...
1. In AntherListen, go to **Settings → Integrations → Spotify**
...
> **Important:** For reliable playback, disable battery optimization for AntherListen in your phone settings (**Settings → Apps → AntherListen → Battery → Unrestricted**). Without this, Android may throttle the app and cause long delays before songs start playing.
...
That's it — the next time you play a song, AntherListen will try Qobuz first and fall back to YouTube Music if the track isn't available there. The toggle can be turned off at any time to revert to YouTube-only playback.
...
### Q: How do I download and install AntherListen?

Go to the [latest release](https://github.com/anagh9090/AntherListen-Online/releases/latest) and download the **AntherListen.apk** file. Open it on your Android device — you may need to allow "Install from unknown sources" in your phone's settings when prompted. You do **not** need to download the source code files.
...
1. **Disable battery optimization for AntherListen** — Go to your phone's **Settings → Apps → AntherListen → Battery → Unrestricted** (or "No restrictions"). This is the most common fix. Android aggressively throttles background network and CPU usage for battery-optimized apps, which directly impacts AntherListen's stream resolution pipeline. Without this setting, playback may take over a minute to start, especially when the screen is locked.
...
### Q: Does AntherListen work with Bluetooth headphones / AirPods?

Yes. AntherListen streams audio through YouTube Music's infrastructure like any other music player. It works with any audio output device including Bluetooth headphones, AirPods, car stereos, and speakers.

### Q: Why isn't AntherListen showing in Android Auto?
...
No. AntherListen uses Spotify for data only (your library, top tracks, search results) — not for audio streaming. Audio is streamed through YouTube Music. A free Spotify account works perfectly.
...
Not every track exists on Qobuz, and not every track exists at every quality tier. If Qobuz returns only a preview (no full stream available) or all resolvers fail, AntherListen falls back to YouTube Music silently and remembers the result. Less popular tracks, indie releases, and rare regional versions are the most common cases. The fallback is the intended behavior and the audio will keep playing — just not in FLAC for that specific track.

### Q: Can my Spotify or YouTube account get banned?

**Spotify:** AntherListen uses Spotify's APIs in read-only mode to access your library, playlists, and recommendations. It does **not** stream audio from Spotify, generate artificial plays, or modify your account data. While using unofficial API clients technically falls outside Spotify's Terms of Service, the risk of account action is considered low — similar apps have operated for years without widespread bans. That said, **use AntherListen at your own risk** and consider using a secondary Spotify account if you're concerned.
...
- Avoid logging into your Google account in AntherListen unless needed for age-restricted content
- Using AntherListen without a Google login carries minimal risk to your Google account
- If you do log in, be aware this carries a small but nonzero risk

**Bottom line:** No bans have been reported by AntherListen users to date. However, as with any third-party client, we cannot guarantee that platform policies won't change in the future.

## Credits

AntherListen is a fork of [Meld](https://github.com/FrancescoGrazioso/Meld/), originally created by [Francesco Grazioso]([https://github.com/mostafaalagamy](https://github.com/FrancescoGrazioso/)).
